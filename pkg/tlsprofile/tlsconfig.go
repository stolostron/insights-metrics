...
func GetTLSConfig(ctx context.Context, dynamicClient dynamic.Interface) (*tls.Config, map[string]interface{}, bool) {
	...
	if err != nil {
		klog.Warning("Could not read APIServer TLS profile, using Intermediate default")
	...
	if len(unsupported) > 0 {
		klog.Warningf("Cipher suites not supported by Go, skipped: %v", unsupported)
	}

	cfg := &tls.Config{} // #nosec G402 - MinVersion set by tlsConfigFn from cluster profile.
	tlsConfigFn(cfg)

	klog.Infof("TLS profile applied: min version TLS 1.%d, %d cipher suites",
	...
	if snapErr != nil {
		klog.Warning("Could not capture TLS profile snapshot for poller baseline")
...
func fetchTLSProfileSpec(ctx context.Context, dynamicClient dynamic.Interface) (*configv1.TLSProfileSpec, error) {
	...
	klog.Infof("Using cluster TLS profile: %s", profile.Type)
	return &resolved, nil
}
...
func IntermediateProfileTLSConfig() *tls.Config {
	...
	if len(unsupported) > 0 {
		klog.Warningf("Intermediate profile: cipher suites not supported by Go: %v", unsupported)
	}
